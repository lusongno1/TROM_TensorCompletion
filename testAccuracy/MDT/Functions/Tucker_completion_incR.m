...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X, G, U, histo, histoR] = Tucker_completion_incR(T,Q,G,U,delta,incR,maxiter,inloop,tol,verb,S,D)
  ...
  for iter = 1:maxiter
    ...
    global Y_true Err
    hold on;
    XestTmp = DE_tensor_all_adjoint(GU,S) ./ D;
    ErrTmp = sqrt(sum((XestTmp-Y_true).^2,'all')/sum(XestTmp.^2,'all'));
    tilde_epsilon = norm(T(Q(:)==1) - GU(Q(:)==1))/norm(T(Q(:)==1));
     err = [ErrTmp,tilde_epsilon] 
     Err(end+1,1:2) = err;
    %scatter(iter,ErrTmp);
    %set(gca,'xscale','log');
    %drawnow;
